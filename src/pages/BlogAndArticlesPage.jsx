...
const BlogAndArticlesPage = () => {
  ...
  return (
    <div className="bg-green-50">
      {/* Header Section */}
       <div className="container mx-auto w-10/12 lg:w-[95%]" style={{ maxWidth: '95%' }}>

        <header className="mentors-hero rounded-lg bg-[#000c34] text-white p-56">
          
          <h1 className="text-3xl font-bold">Blogs & Articles</h1>
        </header>
            ...
            {filteredArticles.map((article) => (
              <Link to={`/articles/${article.id}`} key={article.id}>
                <div className="bg-white rounded-lg shadow-lg flex flex-col h-full p-6 cursor-pointer hover:shadow-xl transition-shadow duration-300">
                  ...
                  <span className="mt-4 text-[#000c34] font-semibold inline-block">
                    Read More
                  </span>
                </div>
              </Link>
