...
const Navbar = () => {
  ...
  const menuItems = ["Home", "Courses", "Blogs&Articles", "About"];
  ...
  const linkClass = (item) => {
    ...
    return `block lg:inline hover:bg-[#000c34] rounded ${isActive ? 'bg-[#000c34] text-white' : ''}`;
  };
...
const Navbar = () => {
  ...
  const linkClass = (item) => {
    const isActive =
      ...
      (location.pathname === "/" && item === "Home");
    return `py-2 px-4 block lg:inline hover:bg-[#003C3C] hover:text-white rounded transition-colors duration-300 ease-in-out ${
      isActive ? "bg-[#003C3C] text-white" : "text-black"
    }`;
  };

  return (
    <header className=" p-6 bg-[#F4FDFB] ">
      <div className="flex justify-between items-center">
        <div className="flex gap-2 items-center">
          <h1 className="hidden lg:block text-[#167063] text-2xl font-bold tracking-wide">
            KnowledgePulse
          </h1>
        </div>
        <div className="flex-grow flex items-center justify-center">
        ...
        <nav className="hidden lg:flex items-center justify-center">
          <ul className="flex space-x-8 items-center">
            {menuItems.map((item, index) => (
              <li key={index} className="flex items-center">
                {item === "Blogs&Articles" ? (
                  <Link to="/blog-articles" className={linkClass(item)}>
                    Blogs&Articles
                  </Link>
                ) : (
                  <Link
            ...
            <li>
              <button className="px-5 py-2 bg-[#003C3C] text-white rounded">
                Sign in
              </button>
            </li>
          </ul>
